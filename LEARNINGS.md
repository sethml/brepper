...
- **Pcurves on periodic faces**: `MakeFace::Add(wire)` does not auto-compute pcurves for non-planar surfaces. After adding wires to a periodic face, use `ShapeFix_Edge::fix_add_p_curve_edge_face_bool_real()` on each edge to compute missing pcurves. Without this, `BRepBuilderAPI_Sewing` discards the pcurves when merging edges, causing `BRepGProp` to report zero face area and wrong solid volume. `ShapeFix_Shell::Perform()` also re-adds pcurves during shell fixing, providing a second safety net.
...
- Volume can be wrong when the solid has pcurve issues on periodic faces (e.g., ball_on_cylinder where the seam edge pcurves are incomplete). Volume comparison should be informational, not a hard error.
