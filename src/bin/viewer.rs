...
fn load_stl(path: &std::path::Path) -> MeshData {
    ...
    let edge_positions = extract_feature_edges(&positions, &indices);
    MeshData {
        positions,
        normals,
        indices,
...
fn load_step(path: &std::path::Path, lin_deflection: f64, ang_deflection: f64) -> MeshData {
    ...
    while explorer.more() {
        ...
        let edges = extract_feature_edges(&face_positions, &face_indices);
...
/// Extract boundary edges and edges where dihedral angle exceeds threshold.
fn extract_feature_edges(positions: &[Vector3<f32>], indices: &[u32]) -> Vec<Vector3<f32>> {
    ...
    for ((a, b), normals) in &edge_faces {
        let is_feature = if normals.len() == 1 {
            ...
            normals[0].dot(normals[1]) < cos_threshold
        };
        if is_feature {
            result.push(positions[*a as usize]);
            result.push(positions[*b as usize]);
        }
    }
    result
}
...
impl WireframeRenderer {
    ...
    fn render(&self, ctx: &Context, camera: &Camera, color: [f32; 4]) {
...
fn main() {
    ...
    let mut control = OrbitControl::new(center, extent * 0.01, extent * 10.0);
    ...
    window.render_loop(move |mut frame_input| {
        ...
        for (wf, color) in &wireframes {
            wf.render(&context, &camera, *color);
        }
