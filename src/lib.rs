...
///
/// Uses `step_control::StepReader` which holds the global STEP I/O mutex
/// for its entire lifetime, serializing access to OCCT's non-thread-safe
/// STEP reader state.
...
///
/// Uses `step_control::StepWriter` which holds the global STEP I/O mutex
/// for its entire lifetime, serializing access to OCCT's non-thread-safe
/// STEP writer state (including `Interface_Static` settings).
pub fn write_step_file(shape: &topo_ds::Shape, path: &str) -> Result<(), String> {
    ...
    let mut writer = step_control::StepWriter::new();
    let progress = message::ProgressRange::new();
