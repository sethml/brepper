...
- `total_area: f64` — Total mesh face area of this hypothesis. Used for O(1) area-based replacement comparisons when a later hypothesis tries to claim faces already assigned to this one.

The algorithm uses BFS region growing, analogous to stage 2.1 but seeded from pairs of adjacent faces (since a single triangle is always planar and cannot determine curvature). A key challenge is that **any two non-coplanar faces define some cylinder** — the cross product of their normals gives an axis, and a circle fit always produces a radius. The algorithm uses multi-seed evaluation with angular coverage validation, and allows BFS growth through faces already assigned to compatible hypotheses (same axis direction) to route around barrier faces (-1). This prevents barriers from splitting what should be a single cylinder region:
**Multi-seed evaluation:**
- Initialize all `cylindrical_hypothesis` to `UNDEDUCED` (-2).
- For each face fi with `cylindrical_hypothesis == -2` (undeduced):
    - Collect **all** valid seed partners: for each neighbor ni of fi (regardless of ni's current hypothesis status), check that ni has a sufficiently different normal (cross product magnitude exceeds `MIN_CROSS_THRESHOLD`, e.g. 0.01), and the dihedral angle between fi and ni does not exceed `--angular-tolerance` (default 17.5°). Additionally, the cylinder fit from the seed pair must succeed and all seed vertices must be within `--vertex-tolerance`. Collect all neighbors that pass these checks.
    - If no valid seed partner found: set face fi to -1 (no hypothesis). Skip to next face.
    ...
    - **Early termination** of redundant trials: during a trial BFS, if a best candidate already exists, check whether the trial is rediscovering the same cylinder. After the trial has accumulated 3+ faces, compare its fitted cylinder parameters against the best candidate's: axis directions nearly parallel (`|a_trial · a_best| > 1 - 1e-6`), radii within `--vertex-tolerance`, and axis-to-axis distance within `--vertex-tolerance`. If parameters match AND every face accepted so far is already in the best candidate's face set, abandon the trial — it can only produce a subset of the existing best. **Additionally**, compare the trial against any *existing committed hypothesis* for the seed face fi (if `cylindrical_hypothesis >= 0`): if the trial is rediscovering the same cylinder as the existing hypothesis (same parameter checks), abandon it.
    - Apply **angular coverage validation** (see below) to the best candidate. If it fails, discard and try the next face.
    - If it passes, **commit** the hypothesis — with area-based replacement: compute the total mesh area of the new hypothesis. For each face in the new hypothesis, only reassign `cylindrical_hypothesis` if (a) the face is currently unassigned (`-2` or `-1`), or (b) the new hypothesis has a greater total area than the face's existing hypothesis. Cache `total_area` in the `CylindricalHypothesis` struct for O(1) comparison. Faces that remain assigned to their old hypothesis are removed from the new hypothesis's face/vertex lists before final storage.
...
- Pop faces from the queue and examine each neighbor ni:
    - If ni is already claimed by this trial, skip.
    - If ni has `cylindrical_hypothesis == -1` (rejected): skip. These are sphere/plane faces that were already evaluated and determined to not be on any cylinder.
    - If ni has `cylindrical_hypothesis >= 0` (assigned to existing hypothesis): check axis compatibility. Compare cross product of the trial's current axis direction with the existing hypothesis's axis direction; if not approximately parallel (cross product magnitude > `angular_tol.sin()`), skip. This prevents BFS from merging different quarter-cylinders that share the same radius but have orthogonal axes (e.g., X-axis and Y-axis cylinders meeting at a sphere corner).
    - If ni has `cylindrical_hypothesis == -2` (unprocessed): eligible for absorption.
...
**Early termination during BFS**: After each BFS iteration (processing all neighbors of a popped face), if the trial has 3+ faces, check whether it is rediscovering the same cylinder as (a) the best candidate so far for this starting face, or (b) the existing committed hypothesis for the starting face fi (if `cylindrical_hypothesis >= 0`). Same parameter checks as above (axis parallel, radius match, axis distance match, all faces subset). If either match is detected, abandon the trial.
...
**Computational cost**: Each undeduced face tries K valid seed partners (typical K is 2-4). For each, a trial BFS runs but is quickly abandoned via early termination if it rediscovers a known hypothesis. BFS through >=0 faces with axis compatibility check adds a small overhead per neighbor (cross product comparison) but does not increase the number of BFS iterations significantly. The area-based replacement at commit time is O(faces_in_hypothesis) with cached `total_area` lookups.
