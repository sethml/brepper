...
- [x] Unit tests: 5 unit tests (cone detection, false positive rejection for cube/cylinder/sphere) and 50 integration tests covering stage 2.4 hypothesis counts, cone parameter matching against STEP, surface selection counts, and stage 2.4/2.6 compare tests for all cone models.
- [x] Full pipeline tests: simple_cone, cone_cylinder, and nosecone all pass `--compare` through stage 4.1 (volume diffs ~1e-8, distance 0mm). block_with_conical_hole passes through stage 2.6 but fails at stage 3.1 (edge comparison mismatch, topology issue not cone detection). 9 stage 3 tests (topology, face creation, compare at stages 3.1-3.6) and 4 stage 4 tests (output + compare). Total: 375 tests (was 325).
